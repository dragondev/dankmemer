...
class DankClient {
	...
	async master(text) {
		const data = await centra(base).path('master').query({ text }).header('Authorization', this.token).send();
		if (data.statusCode !== 200) return data.text;
		data.body = new MessageAttachment(data.body);
		return data.body;
	}
	...
	async info() {
		const data = await centra(base).path('abandon').query('hey').header('Authorization', this.token).send();
